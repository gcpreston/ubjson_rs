...
mod serde_tests {
    ...
    use ubjson_rs::{UbjsonDeserializer, UbjsonError, UbjsonSerializer};
    ...
    fn test_serialize_deserialize_struct_optional_keys() {
        ...
        let deserializer_without_location =
            UbjsonDeserializer::new(buffer_without_location.as_slice());
        let result_without_location: Event =
            Event::deserialize(deserializer_without_location).unwrap();
        assert_eq!(result_without_location, original_without_location);
    }

    #[test]
    fn test_serialize_deserialize_multiple_optional_fields() {
        #[derive(Debug, PartialEq, Serialize, Deserialize)]
        struct User {
            id: u32,
            name: String,
        ...
        for original in test_cases {
            ...
            let deserializer = UbjsonDeserializer::new(buffer.as_slice());
            let result: User = User::deserialize(deserializer).unwrap();
            assert_eq!(result, original);
        }
    }
    ...
    fn test_serialize_deserialize_tuple() {
        ...
        let deserializer = UbjsonDeserializer::new(buffer.as_slice());
        let result: (i32, String, bool, f64) =
            <(i32, String, bool, f64)>::deserialize(deserializer).unwrap();
